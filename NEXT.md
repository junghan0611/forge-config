...
| ~~**B. label-remove / label-set 동사 (v2)**~~ | ✅ 해결 (2026-05-29) | — | NEXT 0-A9a. `label-set` 은 상태 라벨군 단일성 보장. `agent:blocked` 라벨은 repo별 생성 전이면 실패하도록 둠 |
...
- ✅ `issue-create` 에 Mattermost thread bridge metadata 박힘 — `--mm-channel <CHID> --mm-root-id <RID> [--mm-account <ID>]` 세 플래그. body 끝에 `<!-- openclaw:mm {"channel_id":"...","root_id":"...","account":"..."} -->` HTML comment + 로컬 SQLite SSOT `~/.openclaw/state/forge-mm-links.sqlite` (key=`<profile>:<repo>#<num>`, idempotent INSERT OR REPLACE). 검증: `--mm-channel` + `--mm-root-id` 둘 다 박혀야, 한쪽만 박히면 exit 2. `--mm-account` default = `forgebot`. body 조립 순서: body → metadata → footer (footer 최후, 봇 식별 패턴 일관성). sqlite3 미설치 시 WARN + 계속 (body metadata 가 canonical). oracle/sandbox#12 라이브 round-trip 통과 — body, SQLite row 모두 정확. SKILL.md / AGENTS.md 짝 갱신. (2026-05-28)
- ✅ NEXT 0-A9a — `comment --body-file` + `label-remove` + `label-set` 박힘 (2026-05-29). `comment ISSUE --body-file PATH|-` 로 멀티라인 코멘트가 footer 자동 조립을 유지한다. `label-remove ISSUE LABEL` 은 이름→ID 조회 후 단일 라벨 제거. `label-set ISSUE LABEL` 은 상태 라벨군(`agent:ready`, `agent:running`, `agent:done`, `agent:blocked`, `human:needs-review`) 중 기존 상태를 제거하고 지정 상태 하나만 남기도록 Forgejo replace-label API(`PUT /issues/{num}/labels`)를 사용한다. 지정 라벨이 repo에 없으면 명확히 실패 — 특히 `agent:blocked` 는 v2 상태를 실제 쓰기 전 repo별 라벨 생성 필요. oracle/sandbox#13 로 comment/file round-trip 및 상태 라벨 단일성 검증 완료. AGENTS.md 사용법 갱신.
- ✅ 공개 문서 재정렬 (2026-05-29) — README 를 "대시보드가 아니라 대화→이슈→1차검토→정렬→집중구현 연결고리" 관점으로 재작성. ROADMAP.md 신설: domain bot / forgebot / owner agent / GLG 역할과 near-term loop 정리. AGENTS.md 상단에 English wake-up brief 추가. 목적: 새 에이전트가 깨어났을 때 forge-config 가 OpenClaw transport 가 아니라 GLG agent ownership connector 임을 즉시 이해하도록.
- ✅ forgebot v2 live cycle 반영 (2026-05-29 오후) — `agent:done` 의미를 "구현 완료"가 아니라 **1차 검토/분류 triage cycle 완료**로 문서화. forgebot 은 dispatcher/recorder 이며 owner agent 호출은 기본 read-only first review. owner 반환값: reality check / owner repo / risk / scope / implementation-needed? / priority / Forgejo comment summary. OpenClaw live hook prompt 는 `state → label-set agent:running → 시나리오 판정 → comment --body-file → label-set final` 로 정렬. 검증: work `glg-bot/sandbox#11` classification-only → `human:needs-review` GREEN, work `glg-bot/voscli#10` owner-agent read-only review → `agent:done` GREEN. 둘 다 상태 라벨 누적 없음. work `glg-bot/{sandbox,voscli,incidentcli}` 에 `agent:blocked` 라벨 생성 완료.
- ✅ openclaw 담당자 짝 박음 (2026-05-28) — `workspace-voc/TOOLS.md` (mm 옵션 전달 룰: `chat_id = channel:<id>` → `--mm-channel <id>`, root_id 결정 순서 `reply_to_id → topic_id → message_id`, 양쪽 박혔을 때만 전달). `workspace-forge/{TOOLS,USER}.md` (회수 순서 SQLite SSOT 우선 → Forgejo API body `<!-- openclaw:mm ... -->` 파싱 fallback, metadata 없으면 thread reply 생략하고 broadcast 만). openclaw NEXT 0-A6 자취 갱신. 다음 검증은 vocbot/forgebot 라이브 thread → issue 생성 → forge agent 가 replyToId 로 원 thread 답장하는 E2E 자리 — 라이브 라운드 통과 시 forge-config 측 mm bridge 자리 완전 닫힘.
...
- ~~**`label-remove` / `label-set` 동사**~~ — **해결 (2026-05-29 NEXT 0-A9a)**. `label-remove ISSUE LABEL` 과 `label-set ISSUE LABEL` 박힘. `label-set` 은 상태 라벨군(`agent:ready`, `agent:running`, `agent:done`, `agent:blocked`, `human:needs-review`) 을 mutual exclusive 로 만든다. 남은 운영 작업: `agent:blocked` 를 실제 상태로 쓰려는 repo에는 라벨을 먼저 생성해야 한다.
